...
function address(personalDetails) {
    var arr = [];
    var streetAddress = get(personalDetails, 'streetAddress', null);
    var addressSecondLine = get(personalDetails, 'addressSecondLine', null);
    var city = get(personalDetails, 'city', null);
    var country = get(personalDetails, 'country', null);
    var county = get(personalDetails, 'county', null);
    var postCode = get(personalDetails, 'postCode', null);
    if (streetAddress) {
...
function getClinicalRecommendations(obj) {
    var focusValue = get(obj, 'clinicalRecommendations.focusValue', null);
    if (!focusValue) {
        return null;
    }
...
function getSectionFourCpr(obj) {
    var cprValue = get(obj, 'clinicalRecommendations.cprValue', null);
    var result = null;
    switch (cprValue) {
        case 'CPRRecommended':
            result = 100;
...
export default (function (obj, patientInfo) {
    var personalDetails = {
    ...
    var seniorClinician;
    var clinicians = [];
    var signaturesArray = get(obj, 'clinicalSignatures.signaturesArray', []);
    ...
    var defaultDate = moment().format(DATE_FORMAT);
    var dateCompleted = get(form, 'dateCompleted', null) ? form.dateCompleted : defaultDate;
    if (dateCompleted) {
    ...
    var sectionFourCprX = get(form, 'sectionFourCprX', null);
    if (sectionFourCprX) {
    ...
    var sectionSixNotSelectingReason = get(form, 'sectionSixNotSelectingReason', null);
    ...
    var sectionSixDocumentExplanation = get(form, 'sectionSixDocumentExplanation', null);
